...
export function Home(): ReturnType<typeof createElement> {
  useHead({
    ...
    keywords:
      "steamroller, rollup, bundler, javascript, typescript, zero-dependency",
  ...
  return createElement(
    ...
    createElement(
      ...
      createElement(
        ...
        featureCard(
          "Zero Dependencies",
          "No node_modules at runtime. Reduced supply chain risk and smaller install footprint.",
        ),
        featureCard(
          "Pure TypeScript",
          "No native code, no WASM, no build-time code generation. Runs anywhere Node.js runs.",
        ),
        featureCard(
          "Drop-in Compatible",
          "Same API, same types, same plugin system. Migrate from rollup with a one-line change.",
        ),
        featureCard(
          "MIT Licensed",
          "All code is original. No license conflicts, no legal concerns.",
        ),
      ),
    ),
  );
